...
exports.augmentURL = function(url, fieldValuesAsText, filterFields, selectFields, expandFields, orOperator, andOrOperator) {
  ...
  if (filterFields !== null && filterFields.hasOwnProperty('length')) {
    ...
    if (filterFields.length === 1) {
      try {
        filterField = JSON.parse(JSON.stringify(filterFields[0]));
      }
      catch (e) {}
      filterString = filterField['field'] + '%20' + filterField['condition'] + '%20' + "'" + filterField['value'] + "'";
    }
    else if (andOrOperator) {
      for (index = 0; index < filterFields.length; index++) {
        try {
          filterField = JSON.parse(JSON.stringify(filterFields[index]));
        }
        catch (e) {}
        if (index === 0) {
          filterString = '(' + filterField['field'].trim() + '%20' + filterField['condition'] + '%20' + "'" + filterField['value'].trim() + "')";
        }
        else if (index === 1) {
          filterString += '%20and%20((' + filterField['field'].trim() + '%20' + filterField['condition'] + '%20' + "'" + filterField['value'].trim() + "')";
        }
        else {
          filterString += '%20or%20(' + filterField['field'].trim() + '%20' + filterField['condition'] + '%20' + "'" + filterField['value'].trim() + "')";
        }
      }
      ...
      for (i = 0; i < filterFields.length; i++) {
        try {
          filterField = JSON.parse(JSON.stringify(filterFields[i]));
        }
        catch (e) {}
        if (i === 0) {
          filterString = '(' + filterField['field'].trim() + '%20' + filterField['condition'] + '%20' + "'" + filterField['value'].trim() + "')";
